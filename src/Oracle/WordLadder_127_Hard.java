...
public class WordLadder_127_Hard {
    ...
    Space: O(N × M)  M is the length of the word
    Major space usages:
    - 'dict' HashSet: stores N words, each of length M → O(N × M)
    - BFS Queue: in worst case, might store all words → O(N × M)
    - Intermediate char arrays / strings: temporary, but bounded 
      by O(M) per operation.

    So total space complexity is: O(N × M)

	Where: 
	N = number of words in wordList.
	M = length of each word (assuming all words are of the same length).
